...
io.on("connection", (socket) => {
  ...
  function simulateActivityLog(roomIndex, activityLog) {
    let newRoom = structuredClone(rooms[roomIndex])
    let newBones = newRoom.bones;
      ...
      if (entry.action.substr(0,6) === "attack") { //covers "attack", "attacksharp", "attacksharplethal"
        ...
        { //SIGILS - flying, reach, submerge
          ...
          if (newBoard[target].sigils.indexOf("beesonhit") > -1) { //SIGILS - beesonhit
            ...
            let beeCard = { //newHands is never applied - is this all unnecessary and passed by reference????
              card: "bee",
              name: "Bee",
              costType:"bone",
              cost: 0,
            ...
            newHands[target < 4 ? 1 : 0].push(beeCard)
            animationLog.push({action: "updateHand", player: (target < 4 ? 1 : 0), card: structuredClone(beeCard)})
          }
    ...
    //     }}
    // } else {
    //   rooms[roomIndex] = {...newRoom, board: newBoard, scale: newScale, bones: newBones, gameState: (newRoom.gameState === "simulating0" ? "draw1" : "draw0"), animationLog: animationLog} //TEMP
    // }
      rooms[roomIndex].animationLog = animationLog;
  }
